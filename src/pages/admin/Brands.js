...
import { postBrand, getAllBrand, updateBrand, deleteBrand } from "../../api/brandApi.js";
...
const Brands = () => {
  ...
  const fetchBrands = async () => {
    try {
      ...
      const data = response.brand || response;
  ...
  // ── FILTERING & PAGINATION ─────────────────────────────
  // FIX #1: Added "|| ''" to prevent crash if cname is missing
  ...
  // FIX #2: Calculate safe page WITHOUT calling setState during render
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const paginated = filtered.slice((safeCurrentPage - 1) * perPage, safeCurrentPage * perPage);

  // FIX #3: Smart Page Numbers (Sliding Window) so UI doesn't break on 100+ pages
  const getPageNumbers = (current, total) => {
    ...
    if (!showLeftDots && showRightDots) {
      ...
      // Middle format: 1 ... 4 5 [6] 7 8 ... 20
      pages.push(1, '...', leftSibling, current, rightSibling, '...', total);
    } else {
  ...
  const handleFile = (file) => {
    ...
    if (uploadedLogo?.preview && typeof uploadedLogo !== 'string') {
  ...
  const openEdit = (brand) => {
    setFormData({
      bName: brand.bName || '',
      bDescription: brand.bDescription || ''
    });
    
    // Handle existing logo (string URL from server)
    if (brand.bLogo && typeof brand.bLogo === 'string') {
      setUploadedLogo({ preview: brand.bLogo, name: 'Current brand logo' });
    } else if (brand.bLogo?.preview) {
      setUploadedLogo(brand.bLogo);
    } else {
      setUploadedLogo(null);
    }
  ...
  const handleAddSubmit = async (e) => {
    ...
    try { 
      ...
      form.append("name", formData.bName.trim());
      form.append("description", formData.bDescription.trim());

      // Only append if new image is uploaded
      if (uploadedLogo?.file) {
        form.append("logo", uploadedLogo.file);
      }
      ...
      const message = error.response?.data?.message || error.message || "Failed to add brand";
      alert(message);
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateBrand(selectedBrand.id, form);
      ...
      <div style={{ padding: '28px 32px', fontFamily: "'Inter',-apple-system,sans-serif" }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '42px', height: '42px', borderRadius: '12px', background: 'linear-gradient(135deg,#6c5ce7,#a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
            </div>
            <div>
              <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#1a1a3e', margin: 0 }}>Brands</h1>
              <p style={{ fontSize: '13px', color: '#8b8fa3', margin: 0 }}>{brands.length} total brands</p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '18px' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '200px', maxWidth: '340px' }}>
            ...
            <input className="fi" style={{ width: '100%', padding: '9px 14px 9px 38px', border: '1px solid #e2e5f1', borderRadius: '10px', fontSize: '13px', outline: 'none', background: '#fff', fontFamily: 'inherit', boxSizing: 'border-box' }} placeholder="Search brands..." value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', boxShadow: '0 1px 3px rgba(0,0,0,.04)', border: '1px solid #eee', overflow: 'hidden' }}>
          ...
          {loading ? (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={thS}>Logo</th>
                  <th style={thS}>Brand Name</th>
                  ...
                  <tr key={b.id} className="tr" style={{ transition: 'background .15s' }}>
                    <td style={tdS}>
                      <div style={{ width: '80px', height: '45px', borderRadius: '8px', background: '#f4f6fb', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '1px solid #eee' }}>
                        {getImageSrc(b.bLogo) ? 
                          <img src={getImageSrc(b.bLogo)} alt={b.bName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : 
          ...
          {!loading && paginated.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderTop: '1px solid #f0f1f5' }}>
              <span style={{ fontSize: '12px', color: '#8b8fa3' }}>
                Showing {filtered.length === 0 ? 0 : (safeCurrentPage - 1) * perPage + 1}–{Math.min(safeCurrentPage * perPage, filtered.length)} of {filtered.length} brands
              </span>
              <div style={{ display: 'flex', gap: '5px' }}>
                ...
                {getPageNumbers(safeCurrentPage, totalPages).map((pg, idx) => 
                  pg === '...' ? (
                    <span key={`dots-${idx}`} style={{ width: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8b8fa3', fontSize: '14px', userSelect: 'none' }}>…</span>
                  ) : (
                    <button key={pg} className={`pb ${pg === safeCurrentPage ? 'act' : ''}`} style={pgBtn(pg === safeCurrentPage)} onClick={() => setCurrentPage(pg)}>{pg}</button>
                  )
                )}
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d1d5f0" strokeWidth="1.5" style={{ marginBottom: '12px' }}><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>
              <p style={{ fontSize: '15px', fontWeight: 500, color: '#aaa', margin: 0 }}>No brands found</p>
              <p style={{ fontSize: '13px', color: '#ccc', margin: '8px 0 0' }}>Try adjusting your search or add a new brand</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div style={modalS('sm')}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '22px 26px 0' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#1a1a3e', margin: 0 }}>
                {modal === 'add' ? '➕ Add New Brand' : '✏️ Edit Brand'}
              </h2>
            ...
            <form onSubmit={modal === 'add' ? handleAddSubmit : handleEditSubmit}>
              <div style={{ padding: '22px 26px 26px' }}>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <label style={lblS}>Brand Logo</label>
                  <div 
                  ...
                  >
                    ...
                    {isUploading ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <img src={getImageSrc(uploadedLogo)} alt="" style={{ width: '100px', height: '100px', objectFit: 'cover', borderRadius: '12px', border: '2px solid #eee', marginBottom: '8px' }} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ fontSize: '13px', fontWeight: 600, color: '#059669' }}>{uploadedLogo.name || 'Current Logo'}</span>
                        </div>
                      ...
                      <>
                        ...
                        <div style={{ fontSize: '11px', color: '#aaa', marginTop: '3px' }}>PNG, JPG, WEBP (Recommended: 200x200px)</div>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div>
                    <label style={lblS}>Brand Name *</label>
                    <input 
                      className="fi" 
                      style={inpS} 
        ...
        <div style={overlayS} onClick={e => e.target === e.currentTarget && !deleteLoading && setModal(null)}>
          <div style={modalS('sm')}>
            <div style={{ padding: '36px 28px 28px', textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#1a1a3e', margin: '0 0 8px' }}>Delete Brand?</h2>
              <p style={{ fontSize: '13px', color: '#8b8fa3', margin: '0 0 4px' }}>This will permanently remove the brand.</p>
              <p style={{ fontSize: '14px', fontWeight: 600, color: '#555', margin: '0 0 22px', padding: '8px 12px', background: '#fef2f2', borderRadius: '8px', border: '1px solid #fecaca' }}>"{selectedBrand.bName}"</p>
